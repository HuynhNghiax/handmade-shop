...
const BecomeMaker = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setMessage(null);
    try {
      const payload = { bio, skills, portfolio };
            ...
            {skillTags.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {skillTags.map(tag => (
                  <span
                    key={tag}
                    className="bg-pink-100 text-pink-600 px-4 py-1.5 rounded-full text-xs font-bold flex items-center gap-2"
                  >
                    {tag}
